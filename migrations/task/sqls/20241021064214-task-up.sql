...
INSERT INTO public."CREDIT_PACKAGE"
...
-- 2-2. 新增：在 `CREDIT_PURCHASE` 資料表，新增三筆資料：（請使用 name 欄位做子查詢）
    ...
    -- 2. `王小明` 購買 `21 堂組合包方案`
    -- 3. `好野人` 購買 `14 堂組合包方案`
INSERT INTO public."CREDIT_PURCHASE"
(	
	user_id , 
	credit_package_id , 
	purchased_credits, 
...
VALUES
(
	(SELECT id FROM public."USER" WHERE email = 'lee2000@hexschooltest.io'),
	2
),
(
	(SELECT id FROM public."USER" WHERE email = 'muscle@hexschooltest.io'),
	2
),
(
	(SELECT id FROM public."USER" WHERE email = 'starplatinum@hexschooltest.io'),
	2
);
...
INSERT INTO public."COACH_LINK_SKILL"
(
	coach_id,
	skill_id 
)
...
VALUES
(
	(SELECT u.id FROM "USER" u WHERE "email" = 'lee2000@hexschooltest.io'),
...
(
	...
	(
		SELECT course.id FROM "COURSE" AS course 
		JOIN "USER" AS u ON u.id = course.user_id
		WHERE u.email = 'lee2000@hexschooltest.io'
	),
	'2024-11-24 16:00:00',
	'即將授課'
...
SELECT (purchase.total_credit - booking.used_credit) AS remaining_credit
FROM
(
	SELECT 	cp.user_id,
...
(
	...
	AND status != '課程已取消'
	GROUP BY user_id
) booking ON booking.user_id = purchase.user_id;
...
-- 顯示須包含以下欄位： 總營收
SELECT SUM(cpc.price_paid) AS "總營收"
FROM "CREDIT_PURCHASE" AS cpc
INNER JOIN "CREDIT_PACKAGE" AS cpk ON cpc.credit_package_id = cpk.id 
WHERE cpc.purchase_at BETWEEN '2024-11-01 00:00:00' AND '2024-12-01 00:00:00';
...
FROM "COURSE_BOOKING" cb 
WHERE cb.created_at BETWEEN '2024-11-01 00:00:00' AND '2024-12-01 00:00:00'
AND cb.status != '課程已取消';
